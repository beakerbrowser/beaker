    ...
    return posts.map((post, i) => html`
      <div class="border-lightgray" style="margin-bottom: 5px; padding: 5px; overflow: hidden; white-space: nowrap; overflow: hidden;">
        <div style="display: flex; align-items: center; font-size: 10px; margin-bottom: 2px;">
          ...
          <a href="https://beaker.network/${post.drive.url.slice('hyper://'.length)}" style="margin-right: 5px">${post.drive.title}</a>
          <a class="color-lightgray" href="https://beaker.network/${post.url.slice('hyper://'.length)}" style="margin-right: 10px"><small>${timeDifference(post.stat.ctime)}</small></a>
        </div>
    ...
    value: () => {
      return html`
        <div class="border-lightgray" style="padding: 0 10px; margin: 5px 0">
          ...
          ${hits.map(f => html`
            <p>
              <a href="https://beaker.network/${f.url.slice('hyper://'.length)}">
      ...
      if (opts.mutual) {
        return html`
          <div class="border-lightgray" style="padding: 0 10px 10px; margin: 5px 0">
            ...
            ${following.map(f => html`<a href="https://beaker.network/${f.url.slice('hyper://'.length)}">${f.title} <small>${toNiceDomain(f.url)}</small></a><br>`)}
          </div>
        `
      }
      return html`
        <div class="border-lightgray" style="padding: 0 10px 10px; margin: 5px 0">
          ...
          ${followers.map(f => html`<a href="https://beaker.network/${f.url.slice('hyper://'.length)}">${f.title} <small>${toNiceDomain(f.url)}</small></a><br>`)}
        </div>
      `
    }
  }
...
export async function me (opts = {}) {
  ...
  var url = 'https://beaker.network/' + prof.url.slice('hyper://'.length)
  this.page.goto(url, {newTab: opts['new-tab']})
}

export function view (opts = {}, id) {
  if (isNumeric(id)) id = lastPosts[id - 1]
  if (id.startsWith('hyper://')) id = id.slice('hyper://'.length)
