...
//(cd app && HOME=~/.electron-gyp npm rebuild --runtime=electron --target=11.0.0-beta.11 --disturl=https://atom.io/download/atom-shell --build-from-source); gulp build
...
gulp.task('rebuild', gulp.series(async () => {
  ...
  for (let mod of MODULES_NEEDING_REBUILD) {
    await runAsync(`npm rebuild ${mod} --runtime=electron --target=11.0.0-beta.11 --disturl=https://atom.io/download/atom-shell --build-from-source`, {cwd, env, shell: true})
  }
